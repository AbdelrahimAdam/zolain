...
import { recordingService, isValidYouTubeUrl, extractYouTubeId, getYouTubeEmbedUrl } from '../../services/recordingService.jsx'
...
const Input = React.forwardRef(({ label, type = 'text', placeholder, error, disabled, icon: Icon, suffix, required, autoFocus = false, ...props }, ref) => (
...
const TextArea = React.forwardRef(({ label, placeholder, error, disabled, rows = 3, required, autoFocus = false, ...props }, ref) => (
  <div className="space-y-2">
    {label && (
      <label className={`block text-sm font-medium ${error ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
        {label}{required && <span className="text-red-500 ml-1">*</span>}
...
const Select = ({ label, options, value, onChange, placeholder, error, disabled, icon: Icon }) => {
  return (
    <div className="space-y-2">
      {label && (
        <label className={`block text-sm font-medium ${error ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}>
        ...
        <select
          ...
          className={`w-full px-4 py-3 border ${
            error
              ? 'border-red-300 dark:border-red-700 focus:border-red-500 focus:ring-red-500'
              : 'border-gray-300 dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500'
          } rounded-2xl bg-white/50 dark:bg-gray-700/50 text-gray-900 dark:text-white appearance-none focus:outline-none focus:ring-2 transition-all duration-300 ${
            disabled ? 'opacity-50 cursor-not-allowed' : ''
          } ${Icon ? 'pl-10' : ''}`}
        >
...
const CreateLessonModal = ({ onClose, onSuccess, courseId = null, sectionId = null, initialData = {} }) => {
  ...
  useEffect(() => {
    if (watchedValues.videoUrl) {
      const videoId = extractYouTubeId(watchedValues.videoUrl)
      if (videoId) {
        setYoutubePreview({
          isValid: true,
          videoId,
          embedUrl: getYouTubeEmbedUrl(watchedValues.videoUrl),
  ...
  const createVideoMutation = useMutation(
    ...
    },
    {
      onSuccess: (data) => {
        ...
        queryClient.invalidateQueries(['teacherVideos'])
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full max-h-[95vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 rounded-t-2xl z-10">
          ...
          <button
            ...
            className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors disabled:opacity-50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700"
          >
            <X size={24} />
          </button>
        </div>
              ...
              <Input
                ...
                {...register('tags', {
                  validate: (value) => {
                    ...
                    if (tags.some(t => t.length > 20)) return 'Each tag must be less than 20 characters'
                    return true
                  }
                })}
              />
